  ...
  return (
    <div className="animated-fade-in" style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ marginBottom: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 className="neon-text" style={{ fontSize: '28px', fontWeight: '900' }}>Global Registry Hub</h1>
      ...
      <div className="glass" style={{ padding: '24px', marginBottom: '32px', border: '1px solid rgba(0,245,212,0.1)', borderRadius: '16px' }}>
        <div className="registry-search-row" style={{ display: 'flex', gap: '14px', marginBottom: '16px' }}>
          <div style={{ position: 'relative', flex: 1 }}>
            <Search size={19} style={{ position: 'absolute', left: '18px', top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-muted)' }} />
            <input
              type="text"
              ...
              style={{ paddingLeft: '52px', height: '54px' }}
            />
          </div>
          <button onClick={() => fetchPatient()} disabled={loading} className="btn-primary" style={{ padding: '0 24px', height: '54px', display: 'flex', alignItems: 'center', gap: '8px', whiteSpace: 'nowrap' }}>
            <Globe size={18} /> {loading ? 'Fetching...' : 'Query'}
          </button>
        </div>
        
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--color-text-muted)', fontSize: '12px', flexWrap: 'wrap' }}>
            <Info size={14} style={{ flexShrink: 0 }} /> <b>Authenticated Access:</b> Fetching clinical history for <b>#7</b> automatically retrieves data from the main patient hub.
        </div>
      ...
      {patient && (
        <div className="glass animated-fade-in" style={{ padding: '24px', border: '1px solid rgba(0,245,212,0.2)', borderRadius: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', borderBottom: '1px solid var(--color-border)', paddingBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
             <div>
               <div style={{ fontSize: '11px', color: 'var(--color-neon)', fontWeight: 'bold', marginBottom: '6px' }}>RECORDS RETRIEVED: CLOUD SYNCED</div>
               <h2 style={{ fontSize: '28px', fontWeight: '900', color: 'white' }}>{patient.name}</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {records.length > 0 ? records.map((record, idx) => (
              <div key={idx} className="glass" style={{ padding: '24px', position: 'relative', overflow: 'hidden', borderRadius: '16px' }}>
                ...
                <p style={{ fontSize: '13px', color: 'var(--color-text-secondary)', lineHeight: '1.7', wordBreak: 'break-word' }}>{record.report}</p>
              </div>
            )) : (
